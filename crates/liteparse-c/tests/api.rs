...
#[test]
fn empty_extras_borrow_as_null_with_zero_length() {
    let parser = Parser::plain();
    let result = parser.open("sample.pdf").parse(&[]);
    let mut len = 7usize;
    unsafe {
        assert!(liteparse_result_outline(result.0, &mut len).is_null() && len == 0);
        assert!(liteparse_result_annotations(result.0, 0, &mut len).is_null() && len == 0);
        assert!(liteparse_result_blocks(result.0, 0, &mut len).is_null() && len == 0);
...
fn annotations_forward_twin_links_on_parse_and_extract() {
    ...
    assert_eq!(extracted.status, LITEPARSE_STATUS_OK);
    let extracted_annotations = slice(
        unsafe { liteparse_extract_annotations(extracted.handle, 0, &mut len) },
        len,
    );
    assert_eq!(extracted_annotations.len(), 2);

    for annotation in annotations.iter().chain(extracted_annotations) {
...
fn page_geometry_reports_the_visible_box_user_unit_and_rotation() {
    ...
    let selected = document.parse(&[4, 2]);
    assert_eq!(unsafe { liteparse_result_page_count(selected.0) }, 2);
    assert_eq!(unsafe { liteparse_result_page_number(selected.0, 0) }, 2);
    assert_eq!(unsafe { liteparse_result_page_number(selected.0, 1) }, 4);
    assert_eq!(
        unsafe { liteparse_result_page_geometry(selected.0, 0) }
            .geometry
            .rotation_quarter_turns,
        1
    );
    assert_eq!(
        unsafe { liteparse_result_page_geometry(selected.0, 1) }
            .geometry
            .rotation_quarter_turns,
        3
    );
...
#[test]
fn max_pages_truncates_direct_paths() {
...
fn parse_content_projects_caller_text_without_opening_a_document() {
    ...
    let mut labeled = content_page(1, 0);
    labeled.page_label = view(b"iv");
    let labeled_pages = [labeled];
    let mut labeled_content = liteparse_content_default();
    labeled_content.pages = labeled_pages.as_ptr();
    labeled_content.pages_len = labeled_pages.len();
    labeled_content.items = items.as_ptr();
    labeled_content.items_len = items.len();
    let labeled_result = parser.parse_content(&labeled_content);
    assert_eq!(
        view_str(unsafe { liteparse_result_page_label(labeled_result.0, 0) }),
        "iv"
    );
}
...
fn orientation_parser(corrections: &[LiteParsePageOrientationCorrection]) -> Parser {
    Parser::new(|config| {
        config.orientation_corrections = corrections.as_ptr();
        config.orientation_corrections_len = corrections.len();
    })
}

fn page_text(result: &Result, index: usize) -> String {
    view_str(unsafe { liteparse_result_page_text(result.0, index) })
}

#[test]
fn orientation_correction_180_restores_upright_parse() {
    let upright = Parser::plain().open("sample.pdf").parse(&[]);
    let upright_text = page_text(&upright, 0);
    let (upright_w, upright_h) = {
        let size = unsafe { liteparse_result_page_size(upright.0, 0) };
    ...
    let corrected = orientation_parser(&corrections)
        ...
        .parse(&[]);
    assert_eq!(page_text(&corrected, 0), upright_text);
    let size = unsafe { liteparse_result_page_size(corrected.0, 0) };
    assert!((size.width - upright_w).abs() < 0.01);
    assert!((size.height - upright_h).abs() < 0.01);
    // Geometry and extract paths see the same corrected rotation (upright).
    let geometry = unsafe { liteparse_result_page_geometry(corrected.0, 0) };
    assert!(geometry.present);
    assert_eq!(geometry.geometry.rotation_quarter_turns, 0);
}

#[test]
fn orientation_correction_90_restores_upright_parse_and_screenshots() {
    let upright = Parser::plain().open("sample.pdf").parse(&[]);
    let upright_text = page_text(&upright, 0);

    let corrections = [LiteParsePageOrientationCorrection { page: 1, angle: 90 }];
    let parser = orientation_parser(&corrections);
    let document = parser.open("sample_rotated_90cw.pdf");
    let corrected = document.parse(&[]);
    assert_eq!(page_text(&corrected, 0), upright_text);

    // Direct-PDFium extract geometry follows the correction.
    let extracted = unsafe { liteparse_document_extract(document.0, ptr::null(), 0) };
    assert_eq!(extracted.status, LITEPARSE_STATUS_OK);
    let geometry = unsafe { liteparse_extract_page_geometry(extracted.handle, 0) };
    assert!(geometry.present);
    assert_eq!(geometry.geometry.rotation_quarter_turns, 0);
    unsafe { liteparse_extract_free(extracted.handle) };

    // Screenshots render upright dimensions.
...
fn orientation_correction_ignores_out_of_range_and_rejects_bad_angle() {
    ...
    config.bools_values |= LITEPARSE_FLAG_QUIET;
    let bad = [LiteParsePageOrientationCorrection { page: 1, angle: 45 }];
    config.orientation_corrections = bad.as_ptr();
    config.orientation_corrections_len = bad.len();
    let created = unsafe { liteparse_parser_new(&config) };
    assert_eq!(created.status, LITEPARSE_STATUS_INVALID_CONFIG);
    last_error_contains("45");
}
