...
correcting page orientation, searching text, and reading typed parse results.
Caller-supplied pages and layout blocks are accepted through one
parser-level packed content API.

Page geometry (visible box, user unit, rotation) is resolved directly from
the open document. Direct paths apply the configured orientation
corrections before extraction, matching the parser.
...
| Correct page orientation before extraction | Complete through `orientation_corrections` | Per-page `(page, angle)` pairs counter-rotate the page before parse, extract, raw text, page objects, screenshots, and geometry. Non-cardinal angles are `INVALID_CONFIG`; out-of-range pages are ignored like core. |
| Join annotations to structure-tree references by object number | Not exposed | Twin annotations are reported as distinct entries without a join key. |
...
The C tests cover the ABI boundary: handle lifetimes and null handling,
config validation, page selection, caller-supplied content (projection,
supplied blocks, `max_pages`, merged tables), OCR callbacks, screenshots
(including region crops), search ownership, annotations, forms, page
geometry, orientation corrections, raw-text and extract views, unfiltered
page objects, concurrency, and compilation of the generated header from C.
Core algorithms (projection, classification, OCR merging) are covered by the
core crate's own tests and are not re-verified here.
